...
def _memory_adjustment(candidate: dict, memory: dict | None) -> tuple[float, dict]:
    """Return a bounded historical prior adjustment and evidence details."""
    if not memory:
        return 0.0, {}
    family = _topic_family(candidate.get("title", ""), candidate.get("summary", ""))
    dimensions = [
        ("topic_families", family, 4.0),
    ]
    strong_hooks = memory.get("strong_hooks", [])
    for item in strong_hooks[:3]:
        if isinstance(item, dict) and item.get("key"):
            dimensions.append(("hook_types", item["key"], 0.0))
    best = 0.0
    evidence: dict[str, Any] = {"topic_family": family, "matches": []}
    for category, key, _ in dimensions:
        p = _pattern_lookup(memory, category, key)
        if not p:
            continue
        strength = float(p.get("evidence_strength", 0.0))
        mean = float(p.get("posterior_mean", 0.5))
        delta = (mean - 0.5) * 12.0 * strength
        if category == "topic_families":
            best += delta
            evidence["matches"].append({"category": category, "key": key, "delta": round(delta, 2)})
    return max(-10.0, min(10.0, best)), evidence
...
def _obvious_hard_rejection(candidate: dict, recent_hashes: list[str], memory: dict | None) -> str | None:
    ...
    text = f"{title} {summary}".lower()
    if not title.strip():
        return "Missing topic title"
    if len(title.split()) < 3:
        return "Topic too broad — cannot payoff in one sentence"
    if any(x in text for x in ("requires code", "source code", "programming tutorial", "write code")):
        return "Requires charts or code to explain"
    if any(x in text for x in ("press release", "announces", "announcement")) and not any(x in text for x in ("why", "impact", "danger", "surprise")):
        return "Pure announcement with no conflict"
    family = _topic_family(title, summary)
    for rh in recent_hashes:
        if rh and candidate.get("content_hash", "").startswith(rh[:20]):
            return "Duplicate recent topic"
    weak = (memory or {}).get("weak_topic_families", [])
    for item in weak:
        if isinstance(item, dict) and item.get("key") == family and float(item.get("evidence_strength", 0)) >= 0.7:
            return "Historically weak topic family"
    return None
...
def score_candidates(
    ...
    *,
    ...
    if strategy_path.exists():
        try:
            dynamic_strategy = json.loads(strategy_path.read_text(encoding="utf-8"))
    ...
    for c in candidates:
        ...
        text = f"{title} {summary}".lower()
        hotness = float(c.get("normalized_hotness", 50) or 50)
        c["scores"]["freshness"] = max(0, min(100, hotness))

        source_quality_map = {"hackernews": 80, "github_trending": 65, "devto": 55, "wikipedia_otd": 70}
        ...
        c["scores"]["novelty"] = 0 if any(content_hash.startswith(rh[:20]) for rh in recent_hashes if rh) else 75
        ...
        hard_reason = _obvious_hard_rejection(c, recent_hashes, content_memory)
        c["hard_rejection"] = hard_reason
        if hard_reason:
            c["scores"]["hard_rejection"] = 0

        memory_bonus, memory_evidence = _memory_adjustment(c, content_memory)
        c["memory_adjustment"] = round(memory_bonus, 2)
        c["memory_evidence"] = memory_evidence

    batch_size = max(1, int(scoring_cfg.get("llm_batch_size", 10)))
    for c in candidates:
        c["_heuristic_avg"] = sum(c["scores"].values()) / max(1, len(c["scores"]))
    ...
    try:
        ...
        for c in top_batch:
            c["scores"].update({"audience_fit": 50, "curiosity_gap": 50, "story_potential": 50, "visual_potential": 50, "shareability": 50})

    for c in candidates[batch_size:]:
        c["scores"].update({"audience_fit": 40, "curiosity_gap": 40, "story_potential": 40, "visual_potential": 40, "shareability": 40})

    default_weights = {
        "audience_fit": 0.18, "curiosity_gap": 0.16, "story_potential": 0.13,
        "visual_potential": 0.10, "strategy_alignment": 0.12, "freshness": 0.08,
        "specificity": 0.07, "shareability": 0.07, "novelty": 0.05,
        "source_quality": 0.02, "evergreen_value": 0.02,
    }
    ...
    for c in candidates:
        if c.get("hard_rejection"):
            c["total_score"] = 0.0
        else:
            base = sum(c["scores"].get(dim, 50) * weight for dim, weight in w.items())
            c["total_score"] = round(max(0.0, min(100.0, base + float(c.get("memory_adjustment", 0.0)))), 1)
...
def select_best(scored: list[dict[str, Any]], *, min_score: float | None = None, exploration_ratio: float | None = None) -> dict[str, Any] | None:
    cfg = get_config()
    scoring_cfg = cfg.get_path("topic_scoring", {}) or {}
    min_score = float(scoring_cfg.get("min_topic_score", 72)) if min_score is None else min_score
    exploration_ratio = float(scoring_cfg.get("exploration_ratio", 0.20)) if exploration_ratio is None else exploration_ratio
    qualified = [c for c in scored if c.get("total_score", 0) >= min_score and not c.get("hard_rejection")]
    if not qualified:
        LOG.warning("No candidate met minimum score %.1f. Best was: %s (%.1f)", min_score, scored[0].get("title", "?")[:60] if scored else "none", scored[0].get("total_score", 0) if scored else 0)
        return None
    if len(qualified) > 1 and random.random() < exploration_ratio:
        chosen = random.choice(qualified[1:min(5, len(qualified))])
        LOG.info("Exploration pick: %s (%.1f)", chosen.get("title", "")[:50], chosen.get("total_score", 0))
        return chosen
...
def _llm_batch_score(candidates: list[dict], scoring_cfg: dict) -> list[dict]:
    ...
    for i, c in enumerate(candidates):
        candidate_lines.append(f"[{i}] {c.get('title', '')} (source: {c.get('source', '')}, hotness: {c.get('normalized_hotness', 0)})\n    Summary: {str(c.get('summary', ''))[:250]}")
    prompt_path = repo_root() / "prompts" / "topic_scoring.txt"
    template = prompt_path.read_text(encoding="utf-8") if prompt_path.exists() else _DEFAULT_SCORING_PROMPT
    ...
    result = llm.chat_json([{"role": "user", "content": prompt}], max_tokens=int(llm_cfg.get("max_tokens", 3000)), temperature=float(llm_cfg.get("temperature", 0.3)), reasoning_effort=llm_cfg.get("reasoning_effort"))
    raw_scores = result.get("scores", []) if isinstance(result, dict) else []
    if not isinstance(raw_scores, list): raw_scores = []
    out = []
    for i in range(len(candidates)):
        s = raw_scores[i] if i < len(raw_scores) and isinstance(raw_scores[i], dict) else {}
        out.append({k: _clamp(s.get(k, 50)) for k in ("audience_fit", "curiosity_gap", "story_potential", "visual_potential", "shareability")})
    return out
